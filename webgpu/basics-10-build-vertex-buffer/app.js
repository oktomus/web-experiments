...
const triangles = {
    stride: 5 * SizeOfFloat, // 6 floats (4 for position and 2 for uv)
    positionOffset: 0,
    uvOffset: 3 * SizeOfFloat,
...
async function init() {
    ...
    //=> Put positions and uvs in the same buffer.
    const vertexBufferSize = triangles.positions.length + triangles.uvs.length;
    const vertexBufferData = new Float32Array(vertexBufferSize);
    const elementCount = vertexBufferSize / 5;

    for (var i = 0; i < elementCount; i++) {
        const writeIndex = i * 5; // Positions + UVs makes 5 floats by element
        const positionIndex = i * 3; // Positions are stored 3 by 3
        const uvIndex = i * 2;

        vertexBufferData[writeIndex] = triangles.positions[positionIndex];
        vertexBufferData[writeIndex + 1] = triangles.positions[positionIndex + 1];
        vertexBufferData[writeIndex + 2] = triangles.positions[positionIndex + 2];
        vertexBufferData[writeIndex + 3] = triangles.uvs[uvIndex];
        vertexBufferData[writeIndex + 4] = triangles.uvs[uvIndex + 1];
    }

    console.log(vertexBufferData);
